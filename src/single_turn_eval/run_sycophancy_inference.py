...
Usage:
    python src/single_turn_eval/run_sycophancy_inference.py --input data/variants/sycophancy_variants_legal.jsonl --backend vllm --model meta-llama/Llama-3.1-8B-Instruct
    python src/single_turn_eval/run_sycophancy_inference.py --input data/counterfactual_questions_legal.jsonl --prompt baseline --backend litellm --model together_ai/...

Output:
    data/results/single_turn/sycophancy_{model}_{domain}.jsonl
"""
...
def run_sycophancy(
    ...
    os.makedirs(config.SINGLE_TURN_RESULTS_DIR, exist_ok=True)
    ...
    if is_sycophancy and "domain" in df.columns:
        ...
        output_path = os.path.join(
            config.SINGLE_TURN_RESULTS_DIR, f"{model_safe}_{prompt_name}.jsonl"
        )

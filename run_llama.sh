...
python src/dataset_generation/generate_cot.py wrong-cot --domain medical  --backend $BACKEND --model "$MODEL" \
    --baseline "data/results/baseline/baseline_cot_${MODEL_SAFE}_medical.jsonl"
...
python src/single_turn_eval/run_sycophancy_inference.py --input data/variants/sycophancy_variants_legal.jsonl   --backend $BACKEND --model "$MODEL" --max-tokens 2048
python src/single_turn_eval/run_sycophancy_inference.py --input data/variants/sycophancy_variants_medical.jsonl  --backend $BACKEND --model "$MODEL" --max-tokens 2048

# 5. Analysis
python src/single_turn_eval/analyze_results.py sycophancy --file "data/results/single_turn/sycophancy_${MODEL_SAFE}_legal.jsonl"
python src/single_turn_eval/analyze_results.py sycophancy --file "data/results/single_turn/sycophancy_${MODEL_SAFE}_medical.jsonl"
